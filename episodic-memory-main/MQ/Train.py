...
def Train_VSGN(opt):
    ...
    if opt["scheduler"] == 1:
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=opt["train_lr"], epochs=opt["num_epoch"], steps_per_epoch=500)
        #scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=opt["train_lr"], epochs=opt["num_epoch"], steps_per_epoch=opt["step_size"]) # probar parametros
